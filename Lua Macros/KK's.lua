...
function insert_ci(subs, sel)
    ...
    l_2.text =
        "ci = {0}; cn = _G.unicode.len(orgline.text_stripped:gsub(\" \",\"\"))"
    l_3.comment = true
...
function insert_number_fix(subs, sel)
    ...
    l_1.effect = "code once"
    l_1.text =
        "function fix(origin,count) a=origin*(10^count); a_j=a-math.modf(a); if math.abs(a_j)>=0.5 then num_out=math.modf(a+origin/math.abs(origin))/(10^count); else num_out=math.modf(a)/(10^count);end; return num_out; end;"
    subs[-sel[1]] = l_1
end
...
function insert_Yutils_text_to_shape(subs, sel)
    ...
    l_1.text =
        "function shape() text = _G.decode.create_font(line.styleref.fontname, line.styleref.bold, line.styleref.italic, line.styleref.underline, line.styleref.strikeout, line.styleref.fontsize).text_to_shape(syl.text_stripped) text=string.gsub(text,\" c\",\"\") text=_G.Yutils.shape.filter(text, function(x,y) return math.round(x),math.round(y) end) return text end"
    l_2.comment = true
...
function check_jp_cn(subs) -- 检查中日匹配&空格
    for i = 1, #subs do
        ...
        if l.class == "style" then
            current_style_name = l.name
            if string.match(current_style_name, "JP") ~= nil and
                string.match(current_style_name, "OP") == nil and
                string.match(current_style_name, "ED") == nil and
                string.match(current_style_name, "U") == nil and
                string.match(current_style_name, "furigana") == nil -- 判断TEXT-JP
            then
                jp_style_name = current_style_name

            elseif string.match(current_style_name, "JP") ~= nil and
                string.match(current_style_name, "OP") == nil and
                string.match(current_style_name, "ED") == nil and
                string.match(current_style_name, "U") ~= nil and
                string.match(current_style_name, "furigana") == nil -- 判断TEXT-JP-U
            then
                jp_up_style_name = current_style_name

            elseif string.match(current_style_name, "CN") ~= nil and
                string.match(current_style_name, "OP") == nil and
                string.match(current_style_name, "ED") == nil and
                string.match(current_style_name, "U") == nil and
                string.match(current_style_name, "furigana") == nil -- 判断TEXT-CN
            then
                cn_style_name = current_style_name

            elseif string.match(current_style_name, "CN") ~= nil and
                string.match(current_style_name, "OP") == nil and
                string.match(current_style_name, "ED") == nil and
                string.match(current_style_name, "U") ~= nil and
                string.match(current_style_name, "furigana") == nil -- 判断TEXT-CN-U
            then
                cn_up_style_name = current_style_name
            end
        end
    end
    ...
    if buttons == "OK" then
        ...
        for i = 1, #subs do
            local l = subs[i]
            if l.actor ~= nil then
                if string.match(l.actor, "有误") ~= nil and
                    string.match(l.actor, "此行") ~= nil and
                    string.match(l.actor, "IKK") ~= nil then
                    l.actor = ""
                    subs.delete(i)
                    subs[-i] = l
                end
        ...
        for i = 1, #subs do -- 将所有的字幕信息写入独立的表中
            ...
            elseif l.style == jp_up_style and string.match(l.text, "%-%-") ==
                nil -- 处理JP-UP轴
            then
            ...
            elseif l.style == cn_up_style and string.match(l.text, "%-%-") ==
                nil -- 处理CN-UP轴
            then
        ...
        -- 数量比对
        if jpcnt == cncnt and jpupcnt == cnupcnt then
            aegisub.debug.out("中日轴数量相符，共" .. jpcnt .. "+" ..
                                  jpupcnt .. "行，")
            -- 如果数量一致，则进行时间对比
            local lp = 1
            ...
            local wrongtimesel = {}
            for lp = 1, jpcnt do
                if jp_lines_time_start[lp] ~= cn_lines_time_start[lp] or
                    jp_lines_time_end[lp] ~= cn_lines_time_end[lp] then
            ...
            local lp = 1
            for lp = 1, jpupcnt do
                if jp_up_lines_time_start[lp] ~= cn_up_lines_time_start[lp] or
                    jp_up_lines_time_end[lp] ~= cn_up_lines_time_end[lp] then
                    wrongtimecnt = wrongtimecnt + 1
                    table.insert(wrongtimesel, jp_up_lines_num[lp])
                end
            end
            if wrongtimecnt == 0 then
                ...
                for checktext = 1, jpcnt do -- jp
                    if string.match(jp_text[checktext], " ") ~= nil then
                        table.insert(wrongtext, jp_lines_num[checktext])
                        wrongtextcnt = wrongtextcnt + 1 -- 半角空格
                    elseif string.match(jp_text[checktext], "  ") ~= nil then
                        table.insert(wrongtext, jp_lines_num[checktext])
                        wrongtextcnt = wrongtextcnt + 1 -- 双半角空格
                    elseif string.match(jp_text[checktext], "　　") ~= nil then
                        table.insert(wrongtext, jp_lines_num[checktext])
                        wrongtextcnt = wrongtextcnt + 1 -- 双全角空格
                    elseif string.sub(jp_text[checktext],
                                      string.len(jp_text[checktext]) - 2, -1) ==
                        "　" then
                        table.insert(wrongtext, jp_lines_num[checktext])
                        wrongtextcnt = wrongtextcnt + 1 -- 末尾全角空格
                    elseif string.sub(jp_text[checktext],
                                      string.len(jp_text[checktext]), -1) == " " then
                        table.insert(wrongtext, jp_lines_num[checktext])
                        wrongtextcnt = wrongtextcnt + 1 -- 末尾半角空格
                    elseif string.sub(jp_text[checktext],
                                      string.len(jp_text[checktext]) - 5, -1) ==
                        "　‎" then
                        table.insert(wrongtext, jp_lines_num[checktext])
                        wrongtextcnt = wrongtextcnt + 1
                    end -- 全角空格带换行

                end
                local checktext = 1
                for checktext = 1, jpupcnt do -- jp_up
                    if string.match(jp_up_text[checktext], " ") ~= nil then
                        table.insert(wrongtext, jp_up_lines_num[checktext])
                        wrongtextcnt = wrongtextcnt + 1 -- 半角空格
                    elseif string.match(jp_up_text[checktext], "  ") ~= nil then
                        table.insert(wrongtext, jp_up_lines_num[checktext])
                        wrongtextcnt = wrongtextcnt + 1 -- 双半角空格
                    elseif string.match(jp_up_text[checktext], "　　") ~= nil then
                        table.insert(wrongtext, jp_up_lines_num[checktext])
                        wrongtextcnt = wrongtextcnt + 1 -- 双全角空格
                    elseif string.sub(jp_up_text[checktext],
                                      string.len(jp_up_text[checktext]) - 2, -1) ==
                        "　" then
                        table.insert(wrongtext, jp_up_lines_num[checktext])
                        wrongtextcnt = wrongtextcnt + 1 -- 末尾全角空格
                    elseif string.sub(jp_up_text[checktext],
                                      string.len(jp_up_text[checktext]), -1) ==
                        " " then
                        table.insert(wrongtext, jp_up_lines_num[checktext])
                        wrongtextcnt = wrongtextcnt + 1 -- 末尾半角空格
                    elseif string.sub(jp_up_text[checktext],
                                      string.len(jp_up_text[checktext]) - 5, -1) ==
                        "　‎" then
                        table.insert(wrongtext, jp_up_lines_num[checktext])
                        wrongtextcnt = wrongtextcnt + 1
                    end -- 全角空格带换行
                ...
                for checktext = 1, cncnt do -- cn
                    if string.match(cn_text[checktext], "　") ~= nil then
                        table.insert(wrongtext, cn_lines_num[checktext])
                        wrongtextcnt = wrongtextcnt + 1 -- 全角空格
                    elseif string.match(cn_text[checktext], "  ") ~= nil then
                        table.insert(wrongtext, cn_lines_num[checktext])
                        wrongtextcnt = wrongtextcnt + 1 -- 双半角空格
                    elseif string.match(cn_text[checktext], "　　") ~= nil then
                        table.insert(wrongtext, cn_lines_num[checktext])
                        wrongtextcnt = wrongtextcnt + 1 -- 双全角空格
                    elseif string.sub(cn_text[checktext],
                                      string.len(cn_text[checktext]) - 2, -1) ==
                        "　" then
                        table.insert(wrongtext, cn_lines_num[checktext])
                        wrongtextcnt = wrongtextcnt + 1 -- 末尾全角空格
                    elseif string.sub(cn_text[checktext],
                                      string.len(cn_text[checktext]), -1) == " " then
                        table.insert(wrongtext, cn_lines_num[checktext])
                        wrongtextcnt = wrongtextcnt + 1
                    end -- 末尾半角空格
                end
                ...
                for checktext = 1, cnupcnt do -- cnup
                    if string.match(cn_up_text[checktext], "　") ~= nil then
                        table.insert(wrongtext, cn_up_lines_num[checktext])
                        wrongtextcnt = wrongtextcnt + 1 -- 全角空格
                    elseif string.match(cn_up_text[checktext], "  ") ~= nil then
                        table.insert(wrongtext, cn_up_lines_num[checktext])
                        wrongtextcnt = wrongtextcnt + 1 -- 双半角空格
                    elseif string.match(cn_up_text[checktext], "　　") ~= nil then
                        table.insert(wrongtext, cn_up_lines_num[checktext])
                        wrongtextcnt = wrongtextcnt + 1 -- 双全角空格
                    elseif string.sub(cn_up_text[checktext],
                                      string.len(cn_up_text[checktext]) - 2, -1) ==
                        "　" then
                        table.insert(wrongtext, cn_up_lines_num[checktext])
                        wrongtextcnt = wrongtextcnt + 1 -- 末尾全角空格
                    elseif string.sub(cn_up_text[checktext],
                                      string.len(cn_up_text[checktext]), -1) ==
                        " " then
                        table.insert(wrongtext, cn_up_lines_num[checktext])
                        wrongtextcnt = wrongtextcnt + 1
                    end -- 末尾半角空格
                ...
                if wrongtextcnt == 0 then
                    ...
                    if line_with_lable_count ~= 0 then
                        aegisub.debug.out(
                            "\n\n【提示】有" ..
                                tostring(line_with_lable_count) ..
                                "句字幕带标签，请确认中日字幕均已处理。\n已经自动选择有标签的行。")
                    ...
                    for i = 1, wrongtextcnt do
                        ...
                        tempsub.actor = "IKK: 此行空格有误[" ..
                                            tostring(i) .. "/" ..
                                            tostring(wrongtextcnt) .. "]"
                        subs.delete(tempnum)
                ...
                for i = 1, wrongtimecnt do
                    ...
                    local tempsub = subs[tempnum]
                    tempsub.actor = "IKK: 此行时间有误[" .. tostring(i) ..
                                        "/" .. tostring(wrongtimecnt) .. "]"
                    subs.delete(tempnum)
                    subs[-tempnum] = tempsub
                end
                aegisub.debug.out("但有 " .. tostring(wrongtimecnt) ..
...
function jump_to_corresponding_line(subs, sel)
    ...
    for i = 1, #subs do -- 存储所有的行数
        ...
        if l.class == "dialogue" then
            if string.match(l.style, "JP") ~= nil and
                string.match(l.style, "OP") == nil and
                string.match(l.style, "ED") == nil and
                string.match(l.style, "U") == nil -- 判断TEXT-JP
            then
                table.insert(jp_num, i)
                success_count = success_count + 1
            elseif string.match(l.style, "JP") ~= nil and
                string.match(l.style, "OP") == nil and
                string.match(l.style, "ED") == nil and
                string.match(l.style, "U") ~= nil -- 判断TEXT-JP-U
            then
                table.insert(jp_up_num, i)
                success_count = success_count + 1
            elseif string.match(l.style, "CN") ~= nil and
                string.match(l.style, "OP") == nil and
                string.match(l.style, "ED") == nil and
                string.match(l.style, "U") == nil -- 判断TEXT-CN
            then
            ...
            elseif string.match(l.style, "CN") ~= nil and
                string.match(l.style, "OP") == nil and
                string.match(l.style, "ED") == nil and
                string.match(l.style, "U") ~= nil -- 判断TEXT-CN-U
            then
    ...
    if success_count ~= 0 then
        if (string.match(present_style, "JP") ~= nil and
            string.match(present_style, "OP") == nil and
            string.match(present_style, "ED") == nil and
            string.match(present_style, "U") == nil) or
            (string.match(present_style, "JP") ~= nil and
                string.match(present_style, "OP") == nil and
                string.match(present_style, "ED") == nil and
                string.match(present_style, "U") ~= nil) then
            target_num = present_num - jp_first + cn_first

        elseif (string.match(present_style, "CN") ~= nil and
            string.match(present_style, "OP") == nil and
            string.match(present_style, "ED") == nil and
            string.match(present_style, "U") == nil) or
            (string.match(present_style, "CN") ~= nil and
                string.match(present_style, "OP") == nil and
                string.match(present_style, "ED") == nil and
                string.match(present_style, "U") ~= nil) then
...
function cover_time_from_corresponding_line(subs, sel)
    ...
    if sel[2] ~= nil then
        ...
        present_line.margin_l = target_line.margin_l
        present_line.margin_r = target_line.margin_r
        subs.delete(sel[1])
    ...
    if buttons == "OK" then
        ...
        for i = 1, #subs do
            ...
            then
                ...
                jp_marginl[jpcnt] = l.margin_l
                jp_marginr[jpcnt] = l.margin_r
                cntalljp = cntalljp + 1
            ...
            then
                ...
                jp_up_marginl[jpupcnt] = l.margin_l
                jp_up_marginr[jpupcnt] = l.margin_r
                cntalljp = cntalljp + 1
        ...
        if jpcnt + jpupcnt == cncnt then
            ...
            for i = 1, cntalljp do
                if styles[i] == 0 then
                    ...
                    tmp_cn_sub.margin_l = jp_marginl[jp]
                    tmp_cn_sub.margin_r = jp_marginr[jp]
                    ...
                    tmp_cn_sub.margin_l = jp_up_marginl[jp_up]
                    tmp_cn_sub.margin_r = jp_up_marginr[jp_up]
                    subs.delete(cntmpnum)
                    subs[-cntmpnum] = tmp_cn_sub
                end
            end
            aegisub.debug.out("处理完成。共覆盖了" ..
                                  tostring(cntalljp) .. "句字幕。")
            aegisub.debug.out(au_return)
        else
            aegisub.debug.out(
...
function recover_subfont(subs) -- 重置字体子集化
    ...
    for i = 1, #style_line do
        pos = style_line[i]
        if string.match(subs[pos].fontname,
    ...
    if sub_font_num > 0 then
        aegisub.debug.out("\n其中" .. tostring(sub_font_num) ..
                              "个是子集，已经修改为思源黑体M (Source Han Sans SC Medium)")
    elseif sub_font_num == 0 then
        aegisub.debug.out("\n其中没有发现子集字体。")
...
-- 注册插件到菜单
aegisub.register_macro(script_name .. "/去注释", "删除所有的注释行",
                       NO)
aegisub.register_macro(script_name .. "/恢复字体子集化",
                       "撤销字体子集化", recover_subfont)
aegisub.register_macro("Kamigami/日文轴覆盖中文轴", "Kamigami JP&CN",
                       cover_cn_by_jp)
aegisub.register_macro("Kamigami/检查中日轴", "Kamigami JP&CN", check_jp_cn)
aegisub.register_macro("Kamigami/跳转到对应行", "Kamigami Jump Line",
                       jump_to_corresponding_line)
aegisub.register_macro("Kamigami/根据对应行调整时间",
                       "Kamigami Cover time from corresponding",
                       cover_time_from_corresponding_line)
aegisub.register_macro("Auto Template/code line", "添加code line",
                       insert_auto_codeline)
aegisub.register_macro("Auto Template/code syl", "添加code syl",
                       insert_auto_codesyl)
aegisub.register_macro("Auto Template/code once", "添加code once",
                       insert_auto_codeonce)
aegisub.register_macro("Auto Template/template line", "添加template line",
                       insert_auto_template_line)
aegisub.register_macro("Auto Template/template char", "添加template char",
                       insert_auto_template_char)
aegisub.register_macro("Auto Template/template noblank",
                       "添加template noblank", insert_auto_template_noblank)
aegisub.register_macro("Auto Template/template line noblank notext",
                       "添加template line noblank notext",
                       insert_auto_template_line_noblank_notext)
aegisub.register_macro("Auto Template/FirstSyl",
                       "添加FirstSyl和对应Template", insert_firstsyl)
aegisub.register_macro("Auto Template/Counter:ci",
                       "添加计数器ci和对应Template", insert_ci)
aegisub.register_macro("Auto Template/Shape", "添加Shape",
                       insert_shape_code_once)
aegisub.register_macro("Auto Template/Fix", "添加fix函数", insert_number_fix)
aegisub.register_macro("Auto Template/Text to Shape", "文字转图形",
                       insert_Yutils_text_to_shape)
aegisub.register_macro("Auto Template/AutoTags/原版", "原始Autotags",
                       insert_autotags_0)
aegisub.register_macro("Auto Template/AutoTags/Pause", "Pause",
                       insert_autotags_1)
aegisub.register_macro("Auto Template/AutoTags/Delay", "Delay",
                       insert_autotags_2)
